...
class Bootstrap extends Yaf_Bootstrap_Abstract {
    ...
    public function _initConfig() {
        //把配置保存起来
        $this->_config = $arrConfig = Yaf_Application::app()->getConfig();
        Yaf_Registry::set('config', $arrConfig);
        
        /**
         * loading and setting config option;
         */
        $settingConfig = new Yaf_Config_Ini(CONFIG_PATH . DS . 'setting.ini', 'development');
        Yaf_Registry::set('settingConfig', $settingConfig);
        
        //Set Debug
        error_reporting($settingConfig->setting->errorLevel);
        if ($settingConfig->setting->debug) {
            ini_set('display_errors', $settingConfig->setting->debug);
        }
